...
namespace IT.WebServices.Authorization.Discord.Controllers
{
    ...
    public class DiscordController : Controller
    {
        ...
        private async Task<IActionResult> HandleSignIn(DiscordCurrentUser discordUser, OAuthTokenResponse tokens)
        {
            ...
            // Ensure the user is in the guild; if not, add them (requires guilds.join scope)
            if (!string.IsNullOrEmpty(_settings.GuildId))
            {
                try
                {
                    var guildMember = await _client.GetGuildMemberAsync(_settings.GuildId, discordUser.Id);
                    if (guildMember == null)
                        await _client.AddGuildMemberAsync(_settings.GuildId, discordUser.Id, tokens.AccessToken);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Could not add user {DiscordId} to guild", discordUser.Id);
                }
            }
